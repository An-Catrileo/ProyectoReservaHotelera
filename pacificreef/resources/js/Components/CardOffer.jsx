...
const CardOffer = ({ isButton = true }) => {
    ...
    // Si offers no existe o está vacío, mostrar un contenido alternativo
    if (!offers || !offers.length || offers.length < 2) {
        return (
            <div className='md:col-span-2 lg:col-span-1 bg-primary-800 rounded-lg overflow-hidden text-white p-10'>
                <h3 className='text-3xl font-extrabold tracking-wide'>Ofertas Especiales</h3>
                <p className='mt-4'>Próximamente tendremos ofertas exclusivas para usted.</p>
                {isButton && (
