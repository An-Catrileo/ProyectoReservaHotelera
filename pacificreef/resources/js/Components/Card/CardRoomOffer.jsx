...
const CardRoomOffer = ({ room, children }) => {
    // Si no hay habitación o falta el slug, no renderizar nada
    if (!room || !room.slug) {
        return null;
    }
    ...
    return (
        <Card data-aos="fade-up" className='lg:flex overflow-hidden'>
            <div className='w-full lg:w-5/12 xl:w-4/12 overflow-hidden bg-cover bg-center'>
                <img src={room.thumb} alt={room.alt || room.name} className='object-cover object-center w-full h-full xl:h-72 transition-transform hover:scale-110' />
            </div>
