...
const CardRoom = ({ room }) => {
    // Si no hay habitación, no renderizar nada
    if (!room || !room.slug) {
        return null;
    }
    ...
    return (
        <div data-aos="fade-up" key={room.id} className='rounded-lg shadow overflow-hidden bg-white flex flex-col'>
            ...
            <div className='p-6 h-full flex flex-col'>
                <Link href={route('room', { slug: room.slug })} className="text-xl lg:text-2xl font-semibold">
                    {room.name}
                </Link>
                <p className='font-light mt-3 grow'>{room.entry}</p>
