...
export default function NavbarMovil({ navigations }) {
    ...
    return (
        <nav className='bg-gray-900 lg:hidden fixed top-0 w-full z-40 shadow'>
            <div className="mx-auto max-w-7xl px-4 sm:px-6 ">
                <div className="relative flex h-16 items-center justify-between">
                    {/* {open ? (
                        <XMarkIcon onClick={() => setOpen(false)} className=" text-white block h-7 w-7" aria-hidden="true" />
                    ) : (
                        <Bars3Icon onClick={() => setOpen(true)} className=" text-white block h-7 w-7" aria-hidden="true" />
                    )} */}
