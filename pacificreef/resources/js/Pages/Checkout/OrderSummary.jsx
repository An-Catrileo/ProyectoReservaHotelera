...
const OrderSummary = ({ handleClickReservation, formUser }) => {
    ...
    // Si faltan datos esenciales, mostrar mensaje de error
    if (!room || !room.slug || !reservation || !charge) {
        return (
            <Card className='overflow-hidden'>
                <div className="p-8 bg-white">
                    <p className="text-red-500">Error: datos de reserva incompletos. Por favor, vuelva al paso anterior.</p>
                </div>
            </Card>
        );
    }
                ...
                <div className='pb-3'>
                    <ItemOrderSummary title="Habitacion">
                        <div className='text-right'>
                            <Link href={route('room', { slug: room.slug })} className="text-primary-700 font-medium">{room.name}</Link>
                        </div>
                    </ItemOrderSummary>
